...
- [x] 8.4 *(follow-up, folded in)* Retire the superseded interim `bot-harness/` back to `archive/` after closing its remaining coverage gaps in harness mode (wins-by-colour stats + dominant-colour and runaway-pot smells, emote-channel exercise in batches); repoint every live reference (`boom2-benchmarking`/`boom2-compounding`/`boom2-delivery` artifacts, docs, constitution v2.2.1). Player/operator-facing **familiars** naming (Timid Toad / Brash Salamander / Silver-tongued Raven / Scatterbrained Moth; the agent as Homunculus; code ids stay literal) and binary renames: `balance_tester`, `familiar_summoning`, `latency_probe`.
