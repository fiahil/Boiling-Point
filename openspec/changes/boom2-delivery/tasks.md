...
- [ ] 1.2 Add seeded, deterministic bot-harness balance runs to CI.
- [ ] 1.3 Add an agent-harness Claude-as-player smoke to CI.
- [ ] 1.4 Add web-client build + Playwright visual tests (activates with `adopt-pixi-client`).
- [ ] 1.5 Make the full gate a required check on `main` before any deploy.
...
- [ ] 3.1 On green `main`: build + publish the server container and the web-client bundle.
...
- [ ] 4.3 Deploy it as static content in front of `web-client/`.
