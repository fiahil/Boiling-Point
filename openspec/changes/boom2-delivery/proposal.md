...
- **Fuller tests in CI** — extend the gate beyond `fmt`/`clippy`/unit to the full Principle-II gate (constitution v2.0.0): **transport/integration** tests (boot an in-process server), and the **web client** (`clients/web/`) build + Playwright visual tests (once the Pixi client lands). Seeded deterministic **bot-harness** balance runs join the gate when the archived harness is revived — required before boom2 balance ships (§IV). This gates everything below.
...
- `ci-test-layers` — CI extended to transport/integration and web-client visual layers (plus revived-harness balance runs when boom2 balance work revives `archive/bot-harness/`); the gate everything else depends on.
