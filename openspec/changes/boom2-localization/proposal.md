...
- **One canonical source, both clients.** Locale files live in one shared place consumed by `web-client/` and `tui-client/`, with a **CI check that every protocol enum variant has a key in every locale** — a new spell without its translations fails the build.
...
- **Flavor names are translated, not transliterated** (Nightshade → Belladone / Belladona); stable English identifiers stay in code, telemetry, and the harnesses.
...
- `localization` — the client-side ID-keyed locale-table system: the EFIGS + Latin set, the shared canonical source for both clients, the CI key-coverage gate, locale-as-client-preference, translated flavor names, and the pseudo-locale/visual-test story.
...
- **Clients:** both `web-client/` and `tui-client/` consume the shared locale tables and render from codes; an in-client locale switcher.
