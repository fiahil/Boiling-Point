...
- **Relationship to `boom2-delivery`** — its pipeline *runs* the per-merge criterion job and republishes the dashboard; its CI **gate** keeps only a thin deterministic AI-client harness **smoke** (completes, reproducible — no balance-metric assertions).
...
- `balance-study` — on-demand at-scale AI-client harness studies with versioned reproducible reports; purely observational.
