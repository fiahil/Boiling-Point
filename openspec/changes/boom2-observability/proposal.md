...
- **Popularity stats from post-game persistence.** The command center gains a popularity panel — games per day (bar chart), unique/new players per day, a games-by-hour-of-day (UTC) histogram, the returning-player share, and window/lifetime totals — read read-only from the consolidated `game_replays` records, since per-day history outlives the in-process projection. With no database configured the panel degrades to "unavailable" (persistence stays optional).
