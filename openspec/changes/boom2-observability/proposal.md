...
- **Popularity stats from post-game persistence.** The command center gains a popularity panel — games per day (bar chart), unique/new players per day, window and lifetime totals — read read-only from the consolidated `game_replays` records, since per-day history outlives the in-process projection. With no database configured the panel degrades to "unavailable" (persistence stays optional).
