...
**Implementation deviation (accepted, Constitution III).** R1 shipped a *procedural* runtime orchestration rather than a strict typestate machine: the room actor performs the lobby→game handoff at four players, and `game::session::run_game` runs the linear sequence `deal → (modifier → refill → waves → depile → score)×rounds → game-over → Deathmatch-on-tie`. Because that sequence is linear and fully exercised by tests (unit coverage of every engine stage plus a full game played over WebSocket to `GameOver`), illegal transitions do not arise in practice; encoding them as compile errors was deferred as complexity not yet justified. The orchestration is isolated enough that introducing typestate later is a localized change.
